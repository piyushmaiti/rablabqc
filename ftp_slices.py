...
class FTPQCplots:
    ...
    def load_images(self):
        ...
        self.suvr_img = self.load_nii(self.suvr_img)
    ...
    def plot_reference_region_slices(self, axes):
        ...
        sns.heatmap(erodedwm_ref_slices, cmap=cmap_cyan, vmin=0.1, alpha=0.3, cbar=False, mask=erodedwm_ref_slices==0, ax=axes)
        sns.heatmap(ImageProcessor.contour_image(ImageProcessor.mask_image(erodedwm_ref_slices, lower_threshold = 0.1, upper_threshold=erodedwm_ref_slices.max())), cbar = False, cmap = cmap_cyan, vmin = 0.1, ax = axes)

        sns.heatmap(infcblg_ref_slices, cmap=cmap_yellow, vmin=0.1, alpha=0.3, cbar=False, mask=infcblg_ref_slices==0, ax=axes)
        sns.heatmap(ImageProcessor.contour_image(ImageProcessor.mask_image(infcblg_ref_slices, lower_threshold = 0.1, upper_threshold=infcblg_ref_slices.max())), cbar = False, cmap = cmap_yellow, vmin = 0.1, ax = axes)
