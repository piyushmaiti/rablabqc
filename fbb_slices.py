...
class FBBQCplots:
    ...
    def plot_reference_region_slices(self, axes):
        ...
        sns.heatmap(brainstem_ref_slices, cmap=cmap_orange, vmin=0.1, alpha=0.3, cbar=False, mask=brainstem_ref_slices==0, ax=axes)
        sns.heatmap(ImageProcessor.contour_image(ImageProcessor.mask_image(brainstem_ref_slices, lower_threshold = 0.1, upper_threshold=brainstem_ref_slices.max())), cbar = False, cmap = cmap_orange, vmin = 0.1, ax = axes)

        sns.heatmap(cbl_ref_slices, cmap=cmap_blue2, vmin=0.1, alpha=0.3, cbar=False, mask=cbl_ref_slices==0, ax=axes)
        sns.heatmap(ImageProcessor.contour_image(ImageProcessor.mask_image(cbl_ref_slices, lower_threshold = 0.1, upper_threshold=cbl_ref_slices.max())), cbar = False, cmap = cmap_blue2, vmin = 0.1, ax = axes)
